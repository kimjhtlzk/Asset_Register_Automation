...
from gcp_logging_control_list import main  as get_data_GCP
from tencent_logging_control_list import main  as get_data_TENCENT
from aws_logging_control_list import main  as get_data_AWS
...
def main():
    ...
    gcp_loggingfirewall_objects, gcp_loggingaccount_objects, gcp_loggingiam_objects, gcp_loggingkey_objects, gcp_logginginstance_objects = get_data_GCP()
    tencent_loggingfirewall_objects, tencent_loggingaccount_objects, tencent_loggingiam_objects, tencent_loggingkey_objects, tencent_logginginstance_objects = get_data_TENCENT()
    aws_loggingfirewall_objects, aws_loggingaccount_objects, aws_loggingiam_objects, aws_loggingkey_objects, aws_logginginstance_objects = get_data_AWS()

    combined_loggingfirewall_objects    = gcp_loggingfirewall_objects + tencent_loggingfirewall_objects + aws_loggingfirewall_objects
    combined_loggingaccount_objects     = gcp_loggingaccount_objects + tencent_loggingaccount_objects + aws_loggingaccount_objects
    combined_loggingiam_objects         = gcp_loggingiam_objects + tencent_loggingiam_objects + aws_loggingiam_objects
    combined_loggingkey_objects         = gcp_loggingkey_objects + tencent_loggingkey_objects + aws_loggingkey_objects
    combined_logginginstance_objects    = gcp_logginginstance_objects + tencent_logginginstance_objects + aws_logginginstance_objects

    # sheet info
    sheet_id = "1rQ3rQ********************"
    worksheet_names = {
        "LOGS_FIREWALL": combined_loggingfirewall_objects,
        "LOGS_ACCOUNT": combined_loggingaccount_objects,
        "LOGS_IAM": combined_loggingiam_objects,
        "LOGS_KEY": combined_loggingkey_objects,
        "LOGS_INSTANCE": combined_logginginstance_objects
    }

    # upload data to gsheet
    for name, data in worksheet_names.items():
        if data:
            upload_gsheet(
                worksheet_name=name,
                sheet_id=sheet_id,
    ...
    print(f"\nLOGS {BLUE}logic running: {hours}h : {minutes}m : {seconds}s{RESET}\n")
